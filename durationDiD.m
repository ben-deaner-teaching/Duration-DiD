function out = durationDiD(absorbed_time,D,X,t_star,burn_in,extrapolation_end,spec,bootstrap_replications,level,parallel_level,sampling_weights)
    ...
    %Compute weights
    if ~isempty(X)
        weights=covariate_balancing(absorbed_time,X,D,0,sampling_weights).*(sampling_weights/mean(sampling_weights));
    else
    ...
    for b=1:bootstrap_replications
        ...
        if ~isempty(X)
            Xb=X(boot_sample(:,b),:);
            sampling_weightsb=sampling_weights(boot_sample(:,b),:);
            weightsb=covariate_balancing(absorbed_timeb,Xb,Db,1,sampling_weightsb).*(sampling_weightsb/mean(sampling_weightsb));
        else
...
function [weights]=covariate_balancing(absorbed_time,X,D,silent,sampling_weights)
    ...
    % Use sampling-weighted cell frequencies
    wcounts_treated = accumarray(inds(treated), sampling_weights(treated), [max(inds), 1]);
    wcounts_untreated = accumarray(inds(untreated), sampling_weights(untreated), [max(inds), 1]);

    % Weighted totals
    wn_treated = sum(sampling_weights(treated));
    wn_untreated = sum(sampling_weights(untreated));

    % Weights: weighted density ratio for untreated, 1 for treated
    weights = (wcounts_treated(inds) / wn_treated) ./ (wcounts_untreated(inds) / wn_untreated);
    weights(D == 1) = 1;

    % Put zero weight on cells with fewer than two untreated
    % (based on unweighted counts for sample-size reliability)
    ...
    if any((counts_untreated<2)==1) && ~silent
        warning(strcat('Dropped  ',num2str(sum(counts_untreated<2)),' cells out of  ',num2str(size(counts_untreated)),' left with ',num2str(sum(1-censor(D==1))),' treated observations and ',num2str(sum(1-censor(D==0))),' untreated.'))
    end

endfunction out = durationDiD(absorbed_time,D,X,t_star,burn_in,extrapolation_end,spec,bootstrap_replications,level,parallel_level,sampling_weights)
